...
function extractGalleryImages(fields) {
  ...
  return fields.Image.map((image) => resolveImageUrl(image?.url)).filter(Boolean);
}
...
function renderSuggestedMiniCards(records) {
  ...
  return records.map((record) => {
    ...
    const imageUrl =
      ...
      resolveImageUrl(fields['Image']?.[0]?.url) ||
      'https://via.placeholder.com/220x140?text=Property';
...
async function initPropertyDetail() {
  ...
  try {
    ...
    const suggestedRecords = getSuggestedProperties(records, record, 3);
